...
wall = io.imread("WallTest.png")

def rotate_matrix( m ):
    return [[m[j][i] for j in range(len(m))] for i in range(len(m[0])-1,-1,-1)]

#wall = rotate_matrix(wall)
    ...
    def RenderSweep(self, rays):
        ...
        while i < len(rays):
            try:
                ...
                if rays[iOne][0] < closest:
                    closest = rays[iOne][0]
                elif rays[i][0] < closest:
                    closest = rays[i][0]

                if rays[iOne][0] > 6.5:
                    pygame.draw.polygon(self.screen, (0,0,0), corners, 2 if self.debug else 0)
                elif map[rays[i][2][0]][rays[i][2][1]] != 1:
                    pygame.draw.polygon(self.screen, [(int(n - (rays[i][1] * 30) - 20) if n - (rays[i][1] * 30) - 20 > 1 else 1) for n in colors[map[rays[i][2][0]][rays[i][2][1]]]], corners, 2 if self.debug else 0)
                else:
                    for imy, line in enumerate(wall):
                        for imx, pix in enumerate(line):
                            skewed = [skew(corners, (len(wall),len(wall[0])), (imx + n,len(wall) - imy + m)) for n,m in zip([0,1,1,0],[-1,-1,0,0])]
                            pygame.draw.polygon(self.screen, [(int(n - (rays[i][1] * 30)) if n - (rays[i][1] * 30) > 1 else 1) for n in pix], skewed, 2 if self.debug else 0)
            except:
                pass
            i += 1
        return (self.height / 4)/(closest * flatness)

if __name__ == "__main__":
    import protogame
    protogame.mainLaunch('MANUALTEXTURES')
